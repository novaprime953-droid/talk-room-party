...
const AdminRecharge = () => {
  ...
  const updateStatus = async (id: string, status: string) => {
    ...
    else { toast.success(`Request ${status}`); refetch(); }
  };

  return (
    <div>
      <h1 className="font-display font-bold text-2xl text-foreground mb-6">Recharge Records</h1>
      ...
      <div className="bg-card rounded-2xl shadow-card overflow-hidden">
        ...
        {(!requests || requests.length === 0) && (
          <p className="text-center text-muted-foreground text-sm py-8">No recharge requests</p>
        )}
      </div>
    </div>
  );
