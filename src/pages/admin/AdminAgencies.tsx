    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>{agencies?.length ?? 0} agencies</span>
        </div>
      </div>
      ...
      <div className="bg-card rounded-2xl shadow-card overflow-hidden">
                  ...
                  <td className="px-4 py-3">
                    {a.status === "pending" && (
                      <div className="flex gap-1">
                        <motion.button whileTap={{ scale: 0.9 }} onClick={() => updateStatus(a.id, "approved")}
                          className="p-1.5 rounded-lg bg-online/10 text-online hover:bg-online/20">
                          <CheckCircle className="w-3.5 h-3.5" />
                        </motion.button>
                        <motion.button whileTap={{ scale: 0.9 }} onClick={() => updateStatus(a.id, "rejected")}
        ...
        {(!agencies || agencies.length === 0) && (
          <p className="text-center text-muted-foreground text-sm py-8">No agencies found</p>
        )}
      </div>
    </div>
  );
